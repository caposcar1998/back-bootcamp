...
func (r *UserRepository) AddUser(u *models.User) (*models.User, error) {
	stmtIns, err := r.db.Prepare("INSERT INTO user(firstname, lastname, username, email, password) VALUES(?, ?, ?, ?, ?)")
	if err != nil {
		return nil, err
	}
	defer stmtIns.Close()

	res, err := stmtIns.Exec(u.FirstName, u.LastName, u.Username, u.Email, u.Password)
	if err != nil {
		return nil, err
	}
...
func (r *UserRepository) GetUserById(idUser int) (*models.User, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&aux.Id, &aux.FirstName, &aux.LastName,
			&aux.Username, &aux.Email, &aux.Password)
		if err != nil {
			return nil, err
		}
		user = aux
...
func (r *UserRepository) UpdateUser(idUser int, u *models.User) (*models.User, error) {
	stmtIns, err := r.db.Prepare("UPDATE user SET firstname=?, lastname=?, username=?, email=?, password=? WHERE id=?")
	if err != nil {
		return nil, err
	}
	defer stmtIns.Close()

	_, err = stmtIns.Exec(u.FirstName, u.LastName, u.Username, u.Email, u.Password, idUser)
	if err != nil {
		return nil, err
	}
	u.Id = idUser
